...
from APP_LIVRES_164 import app
from APP_LIVRES_164.database.database_tools import DBconnection
from APP_LIVRES_164.erreurs.exceptions import *
from APP_LIVRES_164.genres.gestion_genres_wtf_forms import FormWTFAjouterGenres
from APP_LIVRES_164.genres.gestion_genres_wtf_forms import FormWTFDeleteGenre
from APP_LIVRES_164.genres.gestion_genres_wtf_forms import FormWTFUpdateGenre
...
@app.route("/auteurs_afficher/<string:order_by>/<int:id_genre_sel>", methods=['GET', 'POST'])
def genres_afficher(order_by, id_genre_sel):
    if request.method == "GET":
        try:
            with DBconnection() as mc_afficher:
                if order_by == "ASC" and id_genre_sel == 0:
                    strsql_genres_afficher = """SELECT id_auteur, prenom_auteur, nom_auteur FROM auteur ORDER BY id_auteur ASC"""
...
"""
    ...
    But : Ajouter un genre pour un Livre
...
def genre_delete_wtf():
    data_Livres_attribue_genre_delete = None
    btn_submit_del = None
    ...
    try:
        ...
        if request.method == "POST" and form_delete.validate_on_submit():
            ...
            if form_delete.submit_btn_conf_del.data:
                ...
                data_Livres_attribue_genre_delete = session['data_Livres_attribue_genre_delete']
                print("data_Livres_attribue_genre_delete ", data_Livres_attribue_genre_delete)
            ...
            if form_delete.submit_btn_del.data:
                ...
                str_sql_delete_Livres_genre = """DELETE FROM t_genre_Livre WHERE fk_genre = %(value_id_genre)s"""
                str_sql_delete_idgenre = """DELETE FROM t_genre WHERE id_genre = %(value_id_genre)s"""
                # Manière brutale d'effacer d'abord la "fk_genre", même si elle n'existe pas dans la "t_genre_Livre"
                # Ensuite on peut effacer le genre vu qu'il n'est plus "lié" (INNODB) dans la "t_genre_Livre"
                with DBconnection() as mconn_bd:
                    mconn_bd.execute(str_sql_delete_Livres_genre, valeur_delete_dictionnaire)
        ...
        if request.method == "GET":
            ...
            # Requête qui affiche tous les Livres_genres qui ont le genre que l'utilisateur veut effacer
            str_sql_genres_Livres_delete = """SELECT id_genre_Livre, nom_Livre, id_genre, intitule_genre FROM t_genre_Livre 
                                            INNER JOIN t_Livre ON t_genre_Livre.fk_Livre = t_Livre.id_Livre
                                            INNER JOIN t_genre ON t_genre_Livre.fk_genre = t_genre.id_genre
                                            WHERE fk_genre = %(value_id_genre)s"""

            with DBconnection() as mydb_conn:
                mydb_conn.execute(str_sql_genres_Livres_delete, valeur_select_dictionnaire)
                data_Livres_attribue_genre_delete = mydb_conn.fetchall()
                print("data_Livres_attribue_genre_delete...", data_Livres_attribue_genre_delete)

                # Nécessaire pour mémoriser les données afin d'afficher à nouveau
                # le formulaire "genres/genre_delete_wtf.html" lorsque le bouton "Etes-vous sur d'effacer ?" est cliqué.
                session['data_Livres_attribue_genre_delete'] = data_Livres_attribue_genre_delete
    ...
    return render_template("genres/genre_delete_wtf.html",
                           ...
                           data_Livres_associes=data_Livres_attribue_genre_delete)
